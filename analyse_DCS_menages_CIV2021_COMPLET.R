...
#
#   Script R complet — Mémoire de Master
...
# ---- 4.8 Effets marginaux moyens (AME) -------------------------------------------
...
coords_opt <- coords(roc_logit_obj, "best", best.method = "youden",
                      ret = c("threshold", "sensitivity", "specificity"))

cat("\n── PERFORMANCES AU SEUIL OPTIMAL (Youden, Logit) ──\n")
print(coords_opt)

# Matrice de confusion au seuil optimal
seuil  <- coords_opt$threshold[1]
y_pred <- ifelse(prob_logit >= seuil, 1, 0)
cat("\nMatrice de confusion (seuil =", round(seuil, 4), ") :\n")
print(table(Réel = y_obs, Prédit = y_pred))
...
# ---- 4.10 Export du tableau final (odds ratios) ----------------------------------
...
seuils <- list(
  ...
  list(var = "DCS_40", label = "Seuil 40% (principal)")
)
...
for (s in seuils) {
  ...
  cat("Prévalence :", round(mean(data_robustesse[[s$var]], na.rm = TRUE) * 100, 2), "%\n")
  cat(strrep("=", 70), "\n")
...
cat(strrep("=", 75), "\n")
cat(sprintf("%-30s %15s %15s %15s\n", "Critère", "DCS_10", "DCS_25", "DCS_40"))
cat(strrep("-", 75), "\n")

# Log-vraisemblance pondérée manuelle
ll_pond <- function(mod, design) {
  y <- model.response(model.frame(mod))
  p <- as.numeric(predict(mod, type = "response"))
  w <- weights(design)
  sum(w * (y * log(p + 1e-15) + (1 - y) * log(1 - p + 1e-15)))
}

k <- length(coef(modeles[["DCS_40"]]))
...
criteres <- data.frame(
  ...
  DCS_10  = c(round(ll_DCS_10, 1),
              ...
              round(mean(data_robustesse$DCS_10, na.rm = TRUE) * 100, 2)),
  DCS_25  = c(round(ll_DCS_25, 1),
              round(-2 * ll_DCS_25 + 2 * k, 1),
              round(-2 * ll_DCS_25 + k * log(n), 1),
              round(1 - ll_DCS_25 / ll_null_25, 4),
              round(mean(data_robustesse$DCS_25, na.rm = TRUE) * 100, 2)),
  DCS_40  = c(round(ll_DCS_40, 1),
              round(-2 * ll_DCS_40 + 2 * k, 1),
              round(-2 * ll_DCS_40 + k * log(n), 1),
              round(1 - ll_DCS_40 / ll_null_40, 4),
              round(mean(data_robustesse$DCS_40, na.rm = TRUE) * 100, 2))
)
...
for (i in 1:nrow(criteres)) {
  cat(sprintf("%-30s %15s %15s %15s\n",
              ...
              as.character(criteres$DCS_40[i])))
}
cat(strrep("-", 75), "\n\n")
...
cat(strrep("=", 90), "\n")
cat(sprintf("%-30s %18s %18s %18s\n", "Variable", "DCS 10%", "DCS 25%", "DCS 40%"))
cat(strrep("-", 90), "\n")

vars_affich <- rownames(summary(modeles[["DCS_40"]])$coefficients)
...
for (v in vars_affich) {
  ...
  or_40 <- resultats[["DCS_40"]][resultats[["DCS_40"]]$Variable == v, ]
  ...
  cat(sprintf("  %-28s %18s %18s %18s\n",
              v, fmt(or_10), fmt(or_25), fmt(or_40)))
}
